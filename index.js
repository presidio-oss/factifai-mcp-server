...
var logsDir = path.join(os.homedir(), ".factifai", "logs");
...
var readJUNITXML = (sessionId) => {
  try {
    const reportsDir = path2.join(os2.homedir(), ".factifai", "factifai", sessionId, "reports");
    const files = readdirSync(reportsDir).filter(
      (f) => f.startsWith("test-report-") && f.endsWith(".xml")
    );
    if (files.length === 0) return void 0;
    return readFileSync(path2.join(reportsDir, files[0]), "utf8");
  } catch (error) {
    logger_default.error({ error }, "Error reading JUnit XML");
    return void 0;
  }
};
var hasValidEnv = () => {
  const modelProviderSchema = external_exports.object({
    MODEL_PROVIDER: external_exports.enum(["openai", "bedrock"])
  });
  ...
  if (modelProvider === "openai") {
    const openaiSchema = external_exports.object({
      OPENAI_API_KEY: external_exports.string()
    });
    ...
    const bedrockSchema = external_exports.object({
      AWS_ACCESS_KEY_ID: external_exports.string(),
      AWS_SECRET_ACCESS_KEY: external_exports.string(),
      AWS_DEFAULT_REGION: external_exports.string()
    });
...
var FactifaiSessionManager = class {
...
var processArguments = argv.slice(2);
var isChildProcess = processArguments.includes("--run-browser-task");
var sessionManager = new FactifaiSessionManager();
async function runBrowserTaskInChildProcess(session) {
  return new Promise((resolve, reject) => {
    logger_default.debug({ argv }, "Running browser task in child process");
    const child = spawn(
      argv[0],
      [argv[1], "--run-browser-task", `"${session.task}"`, `"${session.sessionId}"`],
      {
        cwd: path2.join(os2.homedir(), ".factifai"),
    ...
    child.on("spawn", () => {
      ...
      logger_default.debug({ session }, "Child process spawned, updating session status to running");
    });
    child.on("error", (err) => {
      sessionManager.updateSession(session.sessionId, {
      ...
      logger_default.error({ err }, "Error running browser task");
      reject(err);
    });
    child.on("exit", (code, signal) => {
      if (code !== 0) {
        ...
        logger_default.error({ code, signal }, "Child process failed");
        reject(new Error(`Child process failed with code ${code} and signal ${signal}`));
      }
      const result = readJUNITXML(session.sessionId) || `The test is complted but not the report is not available yet, you may check back later.`;
      sessionManager.updateSession(session.sessionId, {
        status: "completed" /* COMPLETED */,
        result
      });
      logger_default.debug({ code, signal }, "Child process completed");
...
var server = new McpServer({
  ...
  title: package_default.description
});
server.registerTool(
  "testWithFactifai",
  {
    ...
    `,
    inputSchema: {
      task: external_exports.string().default(`Navigate to duckduckgo.com and search for 'testing automation'`)
    }
  },
  async ({ task }) => {
...
server.registerTool(
  "listFactifaiSessions",
  {
    ...
    inputSchema: {
      status: external_exports.enum(["pending", "running", "completed", "failed"]).optional()
    }
  },
  async ({ status }) => {
...
if (isChildProcess) {
  logger_default.debug({ argv }, "Executing browser task in child process");
  const task = argv[3];
  ...
  logger_default.debug({ task, sessionId }, "Executing browser task");
  ...
  logger_default.debug({}, "Starting Factifai MCP Server");
